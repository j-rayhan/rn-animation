...
import {
  Image, Animated, Text, View, Dimensions, TouchableOpacity
} from 'react-native';
      ...
      <SafeAreaView style={{ marginTop: SPACING * 4 }}>
        <View style={{ height: IMAGE_HEIGHT * 2.1 }}>
          <Animated.FlatList
            ...
            onMomentumScrollEnd={(ev) => {
              setActivIndex(Math.floor(ev.nativeEvent.contentOffset.x / width));
            }}
        ...
        >
          <TouchableOpacity
            onPress={() => {
              ref?.current?.scrollToOffset({
                offset: (activIndex - 1) * width,
                animated: true
              });
              setActivIndex((index) => (index - 1));
            }}
            disabled={activIndex === 0}
            style={{ opacity: activIndex === 0 ? 0.2 : 1 }}
          >
          ...
          <TouchableOpacity
            ...
            disabled={activIndex === (DATA.length - 1)}
            style={{ opacity: activIndex === (DATA.length - 1) ? 0.2 : 1 }}
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
